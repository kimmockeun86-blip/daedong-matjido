import { Award, Compass, Star, BarChart3, Milestone } from 'lucide-react';
...
export default function Dashboard({ restaurants, visitedRestaurants, onSelectRestaurant }: DashboardProps) {
  ...
  const visitedCount = restaurants.filter(
    r => visitedRestaurants.includes(r.id || '') || visitedRestaurants.includes(r.name)
  ).length;
  const conquestRate = totalCount > 0 ? ((visitedCount / totalCount) * 100).toFixed(1) : '0.0';
      ...
      <div style={{
        ...
        <div className="glass-card" style={{ padding: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
          <div style={{ background: 'rgba(6, 182, 212, 0.1)', padding: '10px', borderRadius: '8px', display: 'flex' }}>
            <Milestone style={{ color: '#06b6d4', width: '20px', height: '20px' }} />
          </div>
          <div>
        ...
        {/* 나의 정복률 */}
        <div className="glass-card" style={{ padding: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
          <div style={{ background: 'rgba(236, 72, 153, 0.1)', padding: '10px', borderRadius: '8px', display: 'flex' }}>
            <Award style={{ color: '#ec4899', width: '20px', height: '20px' }} />
          </div>
          <div>
            <p style={{ color: '#94a3b8', fontSize: '11px', fontWeight: '500' }}>나의 정복률</p>
            <p style={{ color: '#f8fafc', fontSize: '20px', fontWeight: '700', marginTop: '2px' }}>{conquestRate}%</p>
          </div>
        </div>

        {/* 방문한 맛집 */}
        <div className="glass-card" style={{ padding: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
          <div style={{ background: 'rgba(16, 185, 129, 0.1)', padding: '10px', borderRadius: '8px', display: 'flex' }}>
            <Compass style={{ color: '#10b981', width: '20px', height: '20px' }} />
          </div>
          <div>
            <p style={{ color: '#94a3b8', fontSize: '11px', fontWeight: '500' }}>방문한 맛집</p>
            <p style={{ color: '#f8fafc', fontSize: '20px', fontWeight: '700', marginTop: '2px' }}>{visitedCount}개</p>
          </div>
        </div>
      </div>
